...
namespace kasMasjid.Interface
{
    ...
    {
        private Pemasukan pemasukan = new Pemasukan();
        ...
        private void tampilGrid()
        {
            if (cariTxt.Text == "")
            {
                pemasukanDgv.DataSource = pemasukan.TampilkanSemua();
            }
            else
            {
                pemasukanDgv.DataSource = pemasukan.CariDgNama(cariTxt.Text);
            }

            belangBelang(pemasukanDgv);
        }
        ...
        {
            if (!pemasukan.ApakahAda(idTxt.Text))
            {
                pemasukan.setNominal = nominalTxt.Text;
                pemasukan.setKeterangan = keteranganTxt.Text;
                pemasukan.setKategori = kategoriCmb.Text;
                pemasukan.setTanggal = tanggalPick.Value.ToString("yyyy-MM-dd");
            ...
            else
            {
                    ...
                    MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (pemasukan.ubahData(idTxt.Text) > 0)
                    {
        ...
        private void pemasukanDgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow baris = this.pemasukanDgv.Rows[e.RowIndex];
        ...
        private void hapusBtn_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(ids))
            {
                if (MessageBox.Show("Apakah data akan dihapus ?", "KONFIRMASI", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    ...
                    pemasukan.HapusData(idTxt.Text);
        ...
        private void tanggalPick_ValueChanged(object sender, EventArgs e)
        {
