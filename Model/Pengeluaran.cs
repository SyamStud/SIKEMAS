...
namespace kasMasjid.Model
{
    internal class Pengeluaran
    {
        ...
        {
            query = "SELECT p.id, p.nominal, p.keterangan, p.tanggal, j.nama FROM pengeluaran p JOIN kategori j ON p.id_kategori = j.id";
        ...
        public int simpanData()
        {
            ...
            query = "insert into pengeluaran (nominal, keterangan, id_kategori, tanggal) VALUES ('" + _nominal + "', '" + _keterangan + "', " + id_kategori + ", '" + _tanggal + "')";
        ...
        public int ubahData(string id)
        {
            ...
            query = "UPDATE pengeluaran SET keterangan ='" + _keterangan + "', nominal='" + _nominal + "', id_kategori='" + id_kategori + "', tanggal='" + _tanggal + "' WHERE id='" + id + "'";
        ...
        {
            query = "SELECT nama FROM kategori WHERE tipe = 'Pengeluaran'";
            return server.Query(query);
        }

        public DataTable CariDgNama(string keterangan)
        {
            query = "select p.id, p.nominal, p.keterangan, p.tanggal, j.nama from pengeluaran p join kategori j on p.id_kategori = j.id where p.keterangan like '%" + keterangan + "%'";
            return server.Query(query);
        }
    }
}